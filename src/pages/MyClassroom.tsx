            ...
            <div>
              ...
              <div className="flex items-center gap-2 bg-white/10 border border-white/15 rounded-xl px-4 py-2.5">
               ...
               <div className="text-right">
                 <p className="text-lg font-bold text-indigo-400">{classmates.length}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(student => {
            ...
            return (
              <div
                key={student.id}
                className={`glass rounded-2xl p-5 transition-all hover:shadow-md ${
                  isYou ? 'border-indigo-400/50 ring-2 ring-indigo-400/20' : 'border-white/10'
                }`}
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {isYou && (
                        <span className="shrink-0 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400">
                          You
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-500/15 text-amber-400 border border-amber-400/25">
                        {student.karma_points} karma
                      </span>
                      {student.role === 'SUPER_ADMIN' && (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-purple-500/15 text-purple-400 border border-purple-400/25">
                          Admin
                        </span>
                      )}
                    </div>
