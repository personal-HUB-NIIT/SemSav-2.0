...
export default function Upload() {
  ...
  const handleFile = (file: File) => {
    ...
    if (file.size > maxSize) { toast.error('File too large. Maximum size is 20 MB.'); return; }
  ...
  const handleAiExtract = async () => {
    ...
    try {
      ...
      if (data.test_type) { setCategory('TEST'); setTestType(data.test_type); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        const { error: storageError } = await supabase.storage.from('semsav-files').upload(filePath, selectedFile, { upsert: false });
        if (storageError) throw storageError;
        const { data: urlData } = supabase.storage.from('semsav-files').getPublicUrl(filePath);
        publicUrl = urlData.publicUrl;
      }

      const { error: dbError } = await supabase.from('uploads').insert({
        user_id: profile.id, branch_id: profile.branch_id, semester: profile.semester,
        subject_id: subjectId, category, title_syllabus: title,
        ...
        file_url: publicUrl, status: 'UNVERIFIED',
      });
  ...
  const inputClass = 'w-full bg-white/10 border border-white/15 text-white placeholder-gray-500 rounded-xl px-4 py-3 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/30 transition-all';
  const selectClass = 'w-full bg-white/10 border border-white/15 text-white rounded-xl px-4 py-3 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/30 transition-all appearance-none cursor-pointer';
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="glass-strong rounded-2xl p-6 border border-white/10 space-y-4">
            <div>
              ...
              <select value={subjectId} onChange={e => setSubjectId(e.target.value)} className={selectClass}>
                ...
                {subjects.map(s => (
                  <option key={s.id} value={s.id} className="bg-gray-900 text-white">{s.subject_name} ({s.subject_code}){s.is_lab ? ' — Lab' : ''}</option>
                ))}
                {subjects.length === 0 && <option disabled className="bg-gray-900 text-white">No subjects found for your branch/semester</option>}
              </select>
            </div>
            <div>
              ...
              <input type="text" value={title} onChange={e => setTitle(e.target.value)}
                placeholder={category === 'NOTES' ? 'e.g. Unit 3 — Tree Traversals' : category === 'ASSIGNMENT' ? 'e.g. Assignment 2 — Sorting Algorithms' : 'e.g. Unit 1–3, excluding Chapter 5'}
          ...
          {(category === 'ASSIGNMENT' || category === 'TEST') && (
            <div className="glass-strong rounded-2xl p-6 border border-white/10 space-y-4">
              {category === 'TEST' && (
                <div>
                  <label className="block text-gray-400 text-sm font-medium mb-1.5">Test Type</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {TEST_TYPES.map(tt => (
                      <button key={tt.value} type="button" onClick={() => setTestType(tt.value)}
              ...
              {category === 'TEST' && (
                <div>
                  <label className="block text-gray-400 text-sm font-medium mb-1.5">Room No. <span className="text-gray-600">(optional)</span></label>
                  <input type="text" value={roomNo} onChange={e => setRoomNo(e.target.value)} placeholder="e.g. A-201, Main Hall" className={inputClass} />
                </div>
              )}
            </div>
          )}
          ...
          <div className="glass-strong rounded-2xl p-6 border border-white/10">
            <label className="block text-gray-400 text-sm font-medium mb-3">
              Attach File {category === 'NOTES' ? <span className="text-red-400">*</span> : <span className="text-gray-600">(optional)</span>}
            </label>
            <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}
              ...
              <input ref={fileInputRef} type="file" className="hidden" accept=".pdf,.doc,.docx,.ppt,.pptx,.jpg,.jpeg,.png,.zip"
              ...
              {selectedFile ? (
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-center gap-4 mt-2">
                    <button type="button" onClick={e => { e.stopPropagation(); setSelectedFile(null); }}
                      className="text-red-400 hover:text-red-300 text-xs font-medium px-3 py-1.5 rounded-lg bg-red-400/10 hover:bg-red-400/20 transition-colors">Remove file</button>
                    <button type="button" disabled={aiExtracting} onClick={e => { e.stopPropagation(); handleAiExtract(); }}
                      ...
                      {aiExtracting ? (<><div className="w-3 h-3 border-2 border-indigo-400/30 border-t-indigo-400 rounded-full animate-spin" />Extracting...</>)
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-gray-600 text-xs">PDF, Word, PPT, Images, ZIP — max 20 MB</p>
                </div>
              )}
            </div>
          </div>
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? (<><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Uploading...</>)
